...
const fetchChatThread = async (threadId: number): Promise<ThreadResponse> => {
  ...
  try {
    console.log(`Fetching thread ${threadId} from ${getApiUrl(`thread/${threadId}`)}`)
    const response = await fetchWithTimeout(getApiUrl(`thread/${threadId}`), {
      method: "GET",
      headers: {
...
export const useChatThread = (threadId?: number) => {
  return useQuery<ThreadResponse, Error>({
    ...
    queryFn: async () => {
      ...
      try {
        return await fetchChatThread(threadId)
      } catch (error) {
        console.error("Error in useChatThread:", error)
